  ...
  io.of("/api/joinPublicRoom").on("connection", socket => {
    ...
    socket.on("deletePublicMessageEveryone", async (data) => {
      try {
        if (data.id) {
          let msgId = data.id;
          let msgDetails = await PublicRoomMessageSchemaModel.findById(msgId);
          if (msgDetails) {
            //  if(receiver_ids.length > 0){
            //   for(let i=0; i < receiver_ids.length ; i++) {
            //     socket.to(receiver_ids[i]).broadcast.emit("onPublicMsgDeleted", msgId);
            //   }
            //  } 
            socket.to(msgDetails.room_id).broadcast.emit("RoomMessageDelete", data.id);
            await PublicRoomMessageSchemaModel.deleteOne({ _id: data.id });
