...
module.exports = {
    ...
    createUserV2: async (req, res) => {
        ...
        if (!error || 1) {
            ...
            if (user) {
                return res.status(401).json({
                    success: false,
                    message: "The number already exists. Please choose another."
                });
            }
    ...
    updateUser: async (req, res) => {
        const { name, email, password, gender, role, phoneNumber, address, isVerified, status, note, avatarUrl, balance, balanceCurrency, punishment, fixedLocation, number, deviceToken } = req.body;
  ...
  // }
    ...
    getUserByNumber: async (req, res) => {
        if (req.body.number) {
            ...
            const user = await userSchemaModel.findOne({ number: number });
            if (!user) {
                res.status(404).json({ error: true, data: "no user found !" });
            } else {
                res.status(200).json({ error: false, data: user });
            ...
            res.status(400).send({ error: true, data: 'user number required' });
        }
    },
    ...
    getAllLocalUsers: async (req, res) => {
        const userId = req.params.userId;
        // const userList = await userSchemaModel.find({},{
        //     token: 0,
        //     originPassword: 0,
