...
module.exports = {
    ...
    setTopMessage: async (req, res, next) => {
        ...
        const max_top_count = parameterSettings.settings.online_chat_admin_email || 3;
    ...
    sendAnnouncement: async (req, res, next) => {
     ...
     deleteMessage: async (req, res, next) => {
         try {
            ...
            await PublicRoomMessageSchemaModel.findByIdAndRemove(id);
            res.send({ error: false, data: `done ${req.body.id}` })
        } catch (err) {
            res.status(400).send({ error: true, data: `${err}` });
        } 
    },
    getImgSrcV2: async (req, res) => {
        const fileLocation = path.resolve(__dirname, '../uploads/public_chat_messages', req.params.imageName);
        const minifiedfileLocation = path.resolve(__dirname, '../uploads/public_chat_messages/minified', req.params.imageName);
        let fileExist = fs.existsSync(fileLocation)
        if (!fileExist) {
            res.status(403).send("Image doesn't exist");
        }
